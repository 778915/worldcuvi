...
function WorldcupListContent() {
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] max-w-7xl mx-auto px-6 py-12">
      {/* 헤더 */}
      <header className="mb-10">
        <div className="flex items-center gap-2 mb-2">
          <Trophy className="w-6 h-6 text-[var(--accent-2)]" aria-hidden="true" />
      ...
      {/* 검색 + 필터 */}
      <section className="flex flex-col sm:flex-row gap-3 mb-8" aria-label="검색 및 필터">
        <div className="relative flex-1">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--accent-2)]" aria-hidden="true" />
          <input
            id="worldcup-search"
            type="text"
        ...
        <button 
          className="flex items-center gap-2 px-6 py-3.5 bg-zinc-100 dark:bg-zinc-900 border border-black/5 dark:border-white/10 rounded-xl text-[var(--accent-2)] hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-all font-bold"
          aria-label="필터 설정"
        >
      ...
      {/* 카테고리 탭 */}
      <nav className="flex gap-2 overflow-x-auto pb-2 mb-8 scrollbar-hide" aria-label="카테고리 선택">
        {CATEGORIES.map((cat) => (
          <button
            key={cat}
            id={`category-${cat}`}
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[var(--accent-2)] text-sm font-medium">{worldcups.length}개의 월드컵 발견</span>
        </div>
      </div>
      ...
      {loading && worldcups.length === 0 ? (
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" aria-label="월드컵 목록">
          <AnimatePresence mode="popLayout">
            {worldcups.map((wc) => (
              ...
              >
                <article>
                  <PremiumHover className="rounded-[2rem] overflow-hidden h-full">
                    <div className="group relative bg-white dark:bg-zinc-950 border border-black/5 dark:border-white/10 h-full flex flex-col">
                      <VideoThumbnail 
                        videoId={wc.thumbnail_youtube_id} 
                        thumbnailUrl={wc.thumbnail_url} 
                        title={wc.title} 
                      />
                      <Link href={`/worldcup/${wc.id}`} className="p-6 block flex-1">
                        ...
                        <h2 className="font-black text-[var(--accent-2)] text-lg leading-tight group-hover:text-[var(--accent-1)] transition-colors mb-2 line-clamp-2">
                           {wc.title}
                        </h2>
                        <div className="flex items-center gap-2">
                           <div className="w-6 h-6 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-[10px] font-black text-zinc-400 uppercase overflow-hidden">
