    ...
    <header className="fixed top-0 left-0 right-0 z-50">
      ...
      <div className="relative z-[100] max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 absolute left-1/2 -translate-x-1/2 bg-white/50 dark:bg-black/20 backdrop-blur-sm px-2 py-1 rounded-2xl border border-black/5 dark:border-white/5">
          ...
          <Link
            href="/popular"
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all group whitespace-nowrap shrink-0 ${
              pathname === '/popular'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all group shrink-0 ${
              pathname === '/worldcup' || pathname.startsWith('/worldcup/genre')
              ...
              >
                <div className="relative w-5 h-5 flex items-center justify-center">
                  <Bell className={`w-5 h-5 transition-colors duration-300 ${
                    isAutoRinging ? 'animate-bellAlert' : 
              ...
              >
                <div className="relative flex items-center justify-center">
                  <div className={`booster-fill-container ${isBoosterAutoRinging ? 'animate-boosterAlert' : ''}`}>
                    <Zap className={`absolute inset-0 w-5 h-5 transition-opacity group-hover:opacity-0 ${
                      isBoosterAutoRinging ? 'text-red-500 fill-red-500' : ''
                    }`} />
                    <div className={`fill-layer ${activePopover === 'boosters' ? '!translate-y-0' : ''}`}>
                      <Zap className={`absolute inset-0 w-5 h-5 ${
                         activePopover === 'boosters' ? 'text-[var(--accent-1)] fill-[var(--accent-1)]' : ''
                      }`} />
                    </div>
                  </div>
                            ...
                            return (
                              ...
                              >
                                <motion.button
                                  ...
                                  className={`rounded-2xl w-full h-full flex items-center text-left btn-hover hover:z-50 transition-transform origin-center border border-transparent ${
                                    isLarge ? 'p-4 flex-row gap-4' : isMedium ? 'p-3 flex-col justify-between text-center' : 'p-2 flex-col justify-between text-center'
                                  }`}
                                  style={{
                                    backgroundColor: 'color-mix(in srgb, var(--accent-1) 8%, transparent)',
